...
class BaseCMN(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.memory_matrix = nn.Parameter(torch.FloatTensor(args.cmm_size, args.cmm_dim))  # 引入的内存矩阵
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        if len(state) == 0:
            ...
            ys = torch.cat([state[0][0], it.unsqueeze(1).cuda()], dim=1).to('cuda:0') # 原来it.unsqueeze(1)  因为it是cpu上得到的参数，需要将其转换到gpu上，所以需要在其后面加.cuda()
            past = state[1:]
        out, past = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device).to('cuda:0'), past=past,
